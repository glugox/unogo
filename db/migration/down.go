...
func DownTo(db *sql.DB, dir string, version int64, opts ...OptionsFunc) error {
	...
	for {
		...
		if currentVersion == 0 {
			log.Info("goose: no migrations to run. current version: %d\n", currentVersion)
			return nil
		}
		current, err := migrations.Current(currentVersion)
		if err != nil {
			log.Warn("goose: migration file not found for current version (%d), error: %s\n", currentVersion, err)
			return err
		}
...
func downToNoVersioning(db *sql.DB, migrations Migrations, version int64) error {
	...
	log.Info("goose: down to current file version: %d\n", finalVersion)
	return nil
}
